...
{
    /// <summary>
    /// Just enough JSON to answer an <c>s1.call</c>. Strings are the only thing that crosses the Sideload bridge,
    /// so the mod needs a writer; it does not need a parser, because the page has <c>JSON.parse</c> built in and
    /// sends anything structured back the same way.
    ///
    /// Same thirty lines as <c>WhatsDab/Chat/Json.cs</c>. Copied rather than shared: two mods is not yet a library,
    /// and a third consumer is when this moves to <c>Workspace/lib</c>.
    /// </summary>
    internal sealed class Json
    {
        ...
        internal Json Add(string key, long value)
        {
            Separate();
            Quote(key);
            _sb.Append(':').Append(value.ToString(System.Globalization.CultureInfo.InvariantCulture));
            return this;
        }
        ...
        /// ToString(), so a value that is closed twice must not grow a second bracket - a corruption that only
        /// shows up as a parse error on the far side of the bridge.
        /// </summary>
