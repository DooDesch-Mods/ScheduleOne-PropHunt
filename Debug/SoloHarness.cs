        ...
        internal static void PhoneSelfTest()
        {
            ...
            for (int tab = 0; tab < PropHunt.Phone.PhoneScreens.TabLabels.Length; tab++)
            {
                ...
                try
                {
                    ...
                    Core.Log.Msg($"[PropHunt] phphone: tab {tab} \"{PropHunt.Phone.PhoneScreens.TabLabels[tab]}\" built ok " +
                                 $"({rows} root child(ren), {deep} object(s) total).");
                }
                catch (Exception e)
                {
                    Core.Log.Error($"[PropHunt] phphone: tab {tab} \"{PropHunt.Phone.PhoneScreens.TabLabels[tab]}\" THREW - {e}");
                }
